...
expired=$(date -d "${exp}" +"%d %b %Y")
	...
	echo -e "TLS: tls / xtls"
	echo -e ""
	echo -e "Link : vless://$uuid@$domain:8080?security=xtls&flow=xtls-rprx-direct#vless_xtls_${user}"
	echo -e "QR : https://api.qrserver.com/v1/create-qr-code/?size=400x400&data=vless://$uuid@$domain:80?security=xtls&flow=xtls-rprx-direct#vless_xtls_${user}"
	echo -e ""
