...
curl -L https://raw.githubusercontent.com/Apeachsan91/server/main/install-release.sh | bash -s -- install

echo $domain > /usr/local/etc/xray/domain
wget -qO /usr/local/etc/xray/config.json "https://raw.githubusercontent.com/Apeachsan91/server/main/xray_tls_ws_mix-rprx-direct.json"
wget -qO /etc/nginx/conf.d/${domain}.conf "https://raw.githubusercontent.com/Apeachsan91/server/main/web.conf"

sed -i "s/xxx/${domain}/g" /etc/nginx/conf.d/${domain}.conf
sed -i "s/x.x.x.x/${ip}/g" /etc/nginx/conf.d/${domain}.conf

wget -qO web.tar.gz "https://github.com/Apeachsan91/server/raw/main/web.tar.gz"
